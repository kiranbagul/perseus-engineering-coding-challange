...
import routes.{HealthRouter, UserRouter}
...
trait Application extends AppConfig {
  ...
  private implicit val system = ActorSystem("user-service")
  private implicit val executor = system.dispatcher
  private implicit val materializer = ActorMaterializer()

  lazy val server = new Server(Seq(new UserRouter(getRepository()), new HealthRouter()), hostIp, hostPort)
